...
import css from '../../../styles/sections/publications/recent.module.scss'

export default function Recent({ mediumArticles }) {

	const feed 		= mediumArticles.feed
	const publications 	= mediumArticles.items
		...
		<Section classProp="borderBottom">
			<Container spacing={'verticalXXXXLrg'}>
				...
				<section className={css.projects}>
					{
					publications.map( ({ title, pubDate, link, author, thumbnail, categories }, index) => {
						const date = new Date(pubDate).toDateString()
						return (
							<>
							<article key={index} className={css.project}>
								<span className={css.featuredImage}>
									<img src={thumbnail} alt="Publications thumbnail" />
								</span>
		...
		<Section classProp="borderBottom">
			<Container spacing={'verticalXXXXLrg'}>
				<SectionTitle
					title="Recent Publications"
					preTitle="Informative"
					subTitle="We publish some of our works."
				/>
				<section className={css.projects}>
